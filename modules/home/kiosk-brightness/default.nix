...
in
{
  home.kiosk-brightness = {
    homeManager =
      { config, pkgs, ... }:
      let
        brightness-server-script = pkgs.writeText "brightness-server.py" (
          builtins.readFile ./brightness-server.py
        );
      ...
      in
      {
        age.secrets.brightness-server-token = {
          file = "${secretsDir}/secrets/users/public-user/brightness-server-token.env.age";
