{ ... }:
{
  den.aspects.plasma = {
    nixos =
      { config, lib, ... }:
      {
        options.my.de.plasma = {
          kiosk = lib.mkEnableOption "Kiosk mode for Plasma";
          laptop = lib.mkEnableOption "Laptop mode for Plasma";
        };
                ...
                powerdevil = {
                  ...
                  AC =
                    ...
                    in
                    {
                      ...
                      dimDisplay = lib.mkIf (!config.my.de.plasma.kiosk) {
                        enable = true;
                        idleTimeout = dimDisplayIdleTimeout;
                      };

                      turnOffDisplay =
                        if config.my.de.plasma.kiosk then
                          { idleTimeout = "never"; }
                        else
                          {
                      ...
                      autoSuspend =
                        if config.my.de.plasma.laptop then
                          {
                            action = "sleep";
                  ...
                  lowBattery = rec {
                    ...
                    autoSuspend = {
                      action = "sleep";
                      idleTimeout = turnOffDisplay.idleTimeout + 20;
                    };
